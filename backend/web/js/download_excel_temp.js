$(function(){
    ...
    $('.ddfs2 .form-import-tmpl').on('submit', function(e){
        e.preventDefault();
        ...
        var $this = $(this),
            ...
            w = $('.w-list-files'),
            tbody = w.find('tbody'),
            progress = $('.'+$this.attr('data-type')+' .ddfs2 .progress-percent'),
            live = 5000,
            dataType = $this.find('input[name=data_type]').val(),
            // Имя таблицы БД
            tFile = $this.find('select[name=type_file]'),
            load = $this.find('img'),
        ...
        switch(dataType){
            case 'brand-product-group':
                var $image = $('#image-brand-product-group');
                if(tFile.val() == ''){
                    LoadAlert('Внимание','Выберите тип файла',5000,'warning');
                    return;
                }
                break;
            case 'size-manufacturer':
                var $image = $('#image-size-manufacturer');
                break;
            case 'product-nomenclature':
                var $image = $('#image-product-nomenclature');
                break;
            case 'product':
                var $image = $('#image-product');
                break;
        }
...
//            res.html('done<br>'+JSON.stringify(data));
            if(data.status == 200){
                ...
                if(data.duplicates)
                    popUp('.ddfs2',data.duplicates,'warning');
                if(data.not_existing)
                    popUp('.ddfs2',data.not_existing,'warning');
            }else{
